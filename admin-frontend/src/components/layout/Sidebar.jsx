...
export const Sidebar = () => {
  ...
  const navItems = [
    ...
    { id: 'pos', label: 'POS Terminal', icon: ShoppingBag, badge: cart.length > 0 ? `${cart.length} in cart` : null },
    { id: 'products', label: 'Jewelry Catalog', icon: Gem, badge: `${products.length}` },
    { id: 'goldrates', label: 'Daily Metal Rates', icon: TrendingUp },
    { id: 'buyback', label: 'Buyback & Trade-In', icon: Repeat },
    { id: 'gemstones', label: 'Gemstones Vault', icon: Sparkles },
    { id: 'customers', label: 'Customers & VIPs', icon: Users },
    { id: 'suppliers', label: 'Suppliers & Bullion', icon: Truck },
  ];

  return (
    <aside className="w-64 bg-white border-r border-slate-200 flex flex-col shrink-0 min-h-screen text-slate-700 shadow-xs">
      {/* Brand Header */}
      <div className="p-5 border-b border-slate-200 flex items-center gap-3 bg-white">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-500 via-amber-400 to-yellow-300 flex items-center justify-center shadow-md shadow-amber-500/20 text-slate-950">
      ...
      <div className="p-4 border-t border-slate-200 bg-slate-50/50">
        <div className="p-3.5 rounded-xl bg-white border border-amber-200/80 shadow-xs relative overflow-hidden">
          <div className="flex items-center justify-between text-xs mb-1">
            ...
            <span className="text-[10px] text-emerald-700 bg-emerald-100 font-semibold px-1.5 py-0.5 rounded">Audited</span>
          </div>
